...
public:
    ...
    bool binSolve(); //solve the problem using binary search
    void routine(Minisat::Solver *s, int iter_num, bool &resolved);// the routine to solve the problem using member methods
    ...
    //bool VertexCover::binGetModel(Minisat::Solver *s);//get the model using binary search
...
bool VertexCover::binSolve(){
    bool solved = false;
    int lo = 0, hi = ver_num, mid = 0;
    while(lo <= hi){//looping to find a minimum vertex
        mid = (lo + hi) / 2;
        Minisat::Solver s;
        routine(&s, mid, solved);
        if (solved == true){
            hi = mid - 1;
        }
        else{
    ...
    routine(&s, mid, solved);
    if (solved == true){
        bool modeled = getModel(&s);//try to use binGetModel()
        if (modeled)
            print();
    }
    return solved;
...
void VertexCover::routine(Minisat::Solver *s, int iter_num, bool &solved){
    init_variables(s, iter_num);
    build(s, iter_num);
    solved = s->solve();
//    if (solved == true){
//        bool modeled = getModel(&s);//try to use binGetModel()
